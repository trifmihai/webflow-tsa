  ...
  var QUICK_TUNING = {
    ...
    presets: {
      ...
      F: {
        ...
        description: "The TSA reference: a short physical recoil, strong damping and a composed return without elastic bounce.",
        mobileViewport: {
          playback: "timed",
          start: "top 82%",
          end: "bottom 18%",
      ...
      G: {
        ...
        description: "A quieter F variation with the smallest recoil, softer damping and the least visual interruption.",
        mobileViewport: {
          playback: "timed",
          start: "top 82%",
          end: "bottom 18%",
      ...
      H: {
        ...
        description: "A firmer F variation with a quicker contact reversal and a slightly stronger, tightly damped recoil.",
        mobileViewport: {
          playback: "timed",
          start: "top 82%",
          end: "bottom 18%",
      ...
      I: {
        ...
        description: "A heavier F variation with restrained vertical lift, a subtle lateral recoil and the slowest damping.",
        mobileViewport: {
          playback: "timed",
          start: "top 82%",
          end: "bottom 18%",
    ...
    runtimeWindow.Webflow.push(() => {
      ...
      const createController = ({
        ...
        const getCastShadowVars = (pose, phase) => {
          ...
          const phaseEffect = phase === "rebound" ? castShadowEffect.rebound ?? castShadowEffect.anticipation : castShadowEffect[phase];
          return {
            /*
        ...
        const animateToImpact = (options = {}) => {
          ...
          const reboundSettleDuration = reboundSettleEnabled ? motionConfig.reboundSettleDurationSeconds ?? 0.24 : 0;
          const finalSettleDuration = reboundEnabled || motionConfig.settleEnabled === false ? 0 : motionConfig.settleDurationSeconds;
        ...
        const refreshNow = () => {
          ...
          setReactionDefaults();
          renderControllerDebug();
          const shouldPreserveReboundHold = preservedState === "impact" && motionConfig.reboundEnabled === true && (preservedPhase === "rebound-hold" || preservedPhase === "rebound-settle");
          const preservedPose = shouldPreserveReboundHold ? getPostImpactHoldPose() ?? interpolatePose(preservedProgress) : interpolatePose(preservedProgress);
          gsap.set(gavel, {
            x: preservedPose.x,
            y: preservedPose.y,
      ...
      const initializeScope = (scope) => {
        ...
        media.add(MOBILE_QUERY2, () => {
          ...
          const getImpactSequenceDurationMs = () => {
            ...
            const dustEffect = impactEffects.dust;
            const resonanceEffect = impactEffects.resonance;
            const postContactMotionDuration = motion.reboundEnabled === true ? (motion.reboundDurationSeconds ?? 0.19) + (motion.reboundSettleEnabled === false ? 0 : motion.reboundSettleDurationSeconds ?? 0.24) : motion.settleEnabled === false ? 0 : motion.settleDurationSeconds;
            const motionDuration = (motion.anticipationEnabled === false ? 0 : motion.anticipationDurationSeconds) + (motion.strikeDurationSeconds ?? motion.strikeMaxDurationSeconds) + (motion.followThroughEnabled === false ? 0 : motion.followThroughDurationSeconds ?? 0.028) + postContactMotionDuration;
