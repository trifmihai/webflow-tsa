...
(() => {
  ...
  function initGavel() {
    ...
    runtimeWindow.Webflow.push(() => {
      ...
      const renderDebug = (component, gavel, target, geometry) => {
      ...
      const createController = ({
        ...
        const interpolateBetweenPoses = (from, to, progress) => {
          return {
        ...
        const normalizeRange = (progress, start, end) => {
          if (end <= start) {
        ...
        const playScrubImpactReaction = () => {
          ...
          const timeline = gsap.timeline({
            ...
            onComplete: () => {
              ...
              const isStillScrubbing = scrubProgressValue > 1e-3 && scrubProgressValue < 0.999;
        ...
        const resetScrubImpactReaction = () => {
          killImpactReactionTimeline();
          setImpactReactionDefaults();
        };
        ...
        const completeTimeline = (timeline) => {
        ...
        const setScrubProgress = (rawProgress, suppressImpactReaction = false) => {
          ...
          let castShadowFromPhase = "rest";
          let castShadowToPhase = "rest";
          ...
          const movingForward = progress > previousScrubProgress + 1e-4;
          ...
          const isInsideScrub = progress > 1e-3 && progress < 0.999;
          setWillChange(isInsideScrub || Boolean(impactReactionTimeline));
        };
        const animateToImpact = (skipAnticipation = false) => {
          ...
          const canAnticipate = !skipAnticipation && currentProgress <= motionConfig.anticipationMaxProgress;
          if (canAnticipate) {
          ...
          timeline.to(
            ...
            contactTime + duration(targetEffect.compressionDurationSeconds + targetEffect.recoilDurationSeconds)
          );
          timeline.to(
            shadow,
            {
        ...
        const refresh = () => {
          ...
          resizeFrame = requestAnimationFrame(() => {
            ...
            if (scrubModeActive) {
              ...
              scrubImpactTriggered = preservedProgress >= QUICK_TUNING.interaction.mobileScrub.contactProgress;
      ...
      const initializeScope = (scope) => {
        ...
        media.add(MOBILE_QUERY2, () => {
          ...
          const releaseImpactSuppression = () => {
          ...
          const scrollTrigger = ScrollTrigger.create({
            ...
            onRefresh: (self2) => {
              controller.setScrubProgress(self2.progress, true);
      ...
      runtimeWindow.TSAGavel = {
        ...
        listPresets: () => {
          const names = Object.keys(QUICK_TUNING.presets);
          console.table(
            names.map((name) => ({
              preset: name,
